...
// Este es un ejemplo, en un entorno real debes integrar la API de PayPal.

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Aquí harías la validación y conexión con PayPal.
    // Supongamos que la validación es exitosa:
    $pago_exitoso = true; // Simulación de pago exitoso.

    if ($pago_exitoso) {
        // Redirigir a confirmación de pago
        header('Location: confirmacion_pago.php?metodo=paypal');
        exit();
    } else {
        // En caso de error, muestra un mensaje de fallo
        $error_message = "Hubo un problema al procesar tu pago con PayPal. Intenta de nuevo.";
    }
}
...
<body>
    <div class="compra-container">
        ...
        <p>Procesando tu pago a través de PayPal...</p>
        <form method="POST">
            <!-- Simulación de pago: en un entorno real agregarías la integración con la API de PayPal aquí -->
